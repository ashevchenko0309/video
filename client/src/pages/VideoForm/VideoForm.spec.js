...
const DEFAULT_ELEMENTS_VALUES = {
  elements: {
    ...
    thumb: { value: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z/C/HgAGgwJ/lK3Q6wAAAABJRU5ErkJggg==' },
  }
}
  ...
  describe('Validation', () => {
    ...
    describe('Validation Errors', () => {
      it("should set error on submit empty fields", () => {
        ...
        expect(component.state().formError).toBe("Some form field(s) is invalid or empty");
      });
      ...
      it("should show error if video field does not exist", () => {
        ...
        instance.onSubmit({
          preventDefault: () => { },
          currentTarget: { elements: { ...DEFAULT_ELEMENTS_VALUES.elements, video: { files: [] } } }
        })

        expect(component.state().video).toStrictEqual({ hasError: true, errorMessage: 'Must have video file' });
      });

      it("should set form error on server error", () => {
        instance.onFulfilledPost({ errors: [{ param: 'title', msg: 'invalid title' }] });
        expect(component.state().formError).toBe("title - invalid title");
      });

      it("should return error validation on less than minLength", () => {
        instance.onFulfilledPost({ errors: [{ param: 'title', msg: 'invalid title' }] });
        expect(isValidLength('123', { minLength: 5, maxLength: 10 }, 'Error')).toStrictEqual({ hasError: true, errorMessage: 'Error' });
      });

      it("should return error validation on grater than maxLength", () => {
        instance.onFulfilledPost({ errors: [{ param: 'title', msg: 'invalid title' }] });
        expect(isValidLength('12345678910', { minLength: 5, maxLength: 10 }, 'Error')).toStrictEqual({ hasError: true, errorMessage: 'Error' });
      });
    ...
    describe('Validation Success', () => {
      ...
      it("should pass validation on title field", () => {
        ...
        expect(component.state().title).toStrictEqual({ hasError: false, errorMessage: '' });
      });

      it("should pass validation on description field", () => {
        instance.onFieldBlur("World", "description");
        expect(component.state().description).toStrictEqual({ hasError: false, errorMessage: '' });
      });
      ...
      it("should clear errors on change title", () => {
        component.setState({ ...DEFAULT_STATE, title: { hasError: true, errorMessage: 'Title invalid' } });
      ...
      it("should clear errors on change description", () => {
        component.setState({ ...DEFAULT_STATE, description: { hasError: true, errorMessage: 'Title invalid' } });
      ...
      it("should clear errors on change video", () => {
        component.setState({ ...DEFAULT_STATE, video: { hasError: true, errorMessage: 'Video invalid' } });

        instance.onVideoChange();
        expect(component.state().video).toStrictEqual({ hasError: false, errorMessage: '' });
      });

      it("should return success validation", () => {
        instance.onFulfilledPost({ errors: [{ param: 'title', msg: 'invalid title' }] });
        expect(isValidLength('12345', { minLength: 5, maxLength: 10 }, 'Error')).toStrictEqual({ hasError: false, errorMessage: '' });
      });
